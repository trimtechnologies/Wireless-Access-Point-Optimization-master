...
def mw_to_dbm(mw):
    """
    Método que converte a potência recebida dada em mW para dBm  //  Method that converts the given received power in mW to dBm
    :param mw: Valor em miliwatts.                              // :param mw: Value in milliwatts.
    :return: Valor de miliwatts convertido para decibéis.       //  :return: Milliwatt value converted to decibels.
    """
...
def dbm_to_mw(dbm):
    """
    Método que converte a potência recebida dada em dBm para mW. // Method that converts the given received power in dBm to mW.}
    :param dbm: Valor em decibéis.                                 // {:param dbm: Value in decibels.}
    :return: Valor de decibéis convertidos em miliwatts.        //{:return: Decibel value converted to milliwatts.}
    """
    return 10 ** (dbm / 10.)
...
def calc_distance(x1, y1, x2, y2):
    ...
    Method responsible for calculating the distance between two points in the Cartesian plane.
    :param x1: Valor de X no ponto 1.  :param x1: X value at point 1.
    ...
    Returns a float value representing the distance of the points entered
...
def generate_summary(s_array):
    ...
    print("\n****** Generating summaries of the results of simulation ******")
    print("Number of APs:\t" + str(length))
    print("Solution: ", s_array)
    ...
    print("\nSignal Coverage of WI-FI:")
    print("\t" + '{:.2f}'.format(float(percent_cover_above_sensitivity)) + "%\t Good Coverage (Strong Signal)")
    print("\t" + '{:.2f}'.format(
        float(percent_cover_under_sensitivity)) + "%\t Shadow Signal Zones (Below Sensitivity)")
    ...
    print("\n\tCoverage RANGE of signal strength ")
    print("\t\tOptimized Signal  \t" + '{:.1f}'.format(float(percent_range_1)) + "%")
    print("\t\tGood Signal   \t" + '{:.1f}'.format(float(percent_range_2)) + "%")
    print("\t\tBad Signal  \t" + '{:.1f}'.format(float(percent_range_3)) + "%")

    if FOs:
        # Plota gráfico da função objetivo
        print("\n... generating graph of FO behavior.")
        # print("\n... gerando grafico do comportamento da FO.")
        plt.plot(FOs)
        plt.title("Behavior of Simulated Annealing")
...
if __name__ == '__main__':
    ...
    if ANIMATION_STEP_BY_STEP or ANIMATION_BEST_PLACES or ANIMATION_BESTS:
        ...
        pygame.display.set_caption("Simulation Results - IFMG Campus Formiga")
